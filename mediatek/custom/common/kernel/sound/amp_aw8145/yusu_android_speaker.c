...
static bool hac_on=false; //hac open?
...
/*void Sound_Hac_Turnon(void)
{
	...
	mt_set_gpio_mode(GPIO_HAC_EN_PIN, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_HAC_EN_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_HAC_EN_PIN, GPIO_OUT_ONE);
	msleep(SPK_WARM_UP_TIME);
	hac_on = true;
}
...
void Sound_Hac_Turnoff(void)
{
       ...
       mt_set_gpio_mode(GPIO_HAC_EN_PIN, GPIO_MODE_00);
       mt_set_gpio_dir(GPIO_HAC_EN_PIN, GPIO_DIR_OUT);
       mt_set_gpio_out(GPIO_HAC_EN_PIN, GPIO_OUT_ZERO);
       msleep(SPK_WARM_UP_TIME);
       hac_on = false;
}*/
