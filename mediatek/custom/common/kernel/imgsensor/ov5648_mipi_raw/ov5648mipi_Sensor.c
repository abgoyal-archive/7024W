...
//#define OV5648_USE_LENC_OTP   //ov5647t no lens calibration data  //copy form OV5650 LENC OTP

#define RG_TYPICAL 0x16C  //golden sample R/G ratio
#define BG_TYPICAL 0x153  //golden sample B/G ratio
...
kal_uint16 OV5648MIPI_check_otp_wb(kal_uint16 index)
{
	...
	OV5648MIPI_write_cmos_sensor(0x3d21,0x01);

	//read flag
	address = 0x3d05+index*9;
	flag = OV5648MIPI_read_cmos_sensor(address);

	//SENSORDB("OV5648MIPI_check_otp_wb=%d\r\n",flag);

	//do not clear OTP buffer
	//clear otp buffer
	//for(temp=0;temp<32;temp++){
	//	OV5648MIPI_write_cmos_sensor(0x3d00+temp,0x00);
	//}
	
	
	//disable otp read
	OV5648MIPI_write_cmos_sensor(0x3d21,0x0);
	OV5648MIPI_write_cmos_sensor(0x0100, 0x00);	

	if(NULL == flag)
		{
			
			SENSORDB("[OV5648MIPI_check_otp_awb]index[%x]read flag[%x][0]\n",index,flag);
			return 0;
			
		}
	else if(!(flag&0x80) && (flag&0x7f))
		{
			SENSORDB("[OV5648MIPI_check_otp_awb]index[%x]read flag[%x][2]\n",index,flag);
			return 2;
		}
	else
		{
			SENSORDB("[OV5648MIPI_check_otp_awb]index[%x]read flag[%x][1]\n",index,flag);
		    return 1;
		}
...
//index:index of otp group.(0,1,2)
//return: 0
kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp)
{
	kal_uint16 temp;
	kal_uint32 address;
       OV5648MIPI_write_cmos_sensor(0x0100, 0x01);	
	...
	otp->customer_id = (OV5648MIPI_read_cmos_sensor(address)&0x7f);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]module_integrator_id[%x]\n",address,otp->customer_id);
	
	otp->module_integrator_id = OV5648MIPI_read_cmos_sensor(address);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]module_integrator_id[%x]\n",address,otp->module_integrator_id);
	
	otp->lens_id = OV5648MIPI_read_cmos_sensor(address+1);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]lens_id[%x]\n",address,otp->lens_id);
	
	otp->rg_ratio = OV5648MIPI_read_cmos_sensor(address+2);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]rg_ratio[%x]\n",address,otp->rg_ratio);

	otp->bg_ratio = OV5648MIPI_read_cmos_sensor(address+3);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]bg_ratio[%x]\n",address,otp->bg_ratio);

	otp->user_data[0] = OV5648MIPI_read_cmos_sensor(address+4);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]user_data[0][%x]\n",address,otp->user_data[0]);

	otp->user_data[1] = OV5648MIPI_read_cmos_sensor(address+5);
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]user_data[1][%x]\n",address,otp->user_data[1]);

	otp->user_data[2] = OV5648MIPI_read_cmos_sensor(address+6);	
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]user_data[2][%x]\n",address,otp->user_data[2]);

	otp->user_data[3] = OV5648MIPI_read_cmos_sensor(address+7);	
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]user_data[3][%x]\n",address,otp->user_data[3]);

	otp->user_data[4] = OV5648MIPI_read_cmos_sensor(address+8);	
	SENSORDB("[OV5648MIPI_read_otp_wb]address[%x]user_data[3][%x]\n",address,otp->user_data[3]);

	//disable otp read
	OV5648MIPI_write_cmos_sensor(0x3d21,00);
...
kal_uint16 OV5648MIPI_update_wb_gain(kal_uint32 R_gain, kal_uint32 G_gain, kal_uint32 B_gain)
{
    ...
    SENSORDB("[OV5648MIPI_update_wb_gain]R_gain[%x]G_gain[%x]B_gain[%x]\n",R_gain,G_gain,B_gain);
...
//call this function after OV5648MIPI initialization
//return value:	0 update success
//				1 no 	OTP

kal_uint16 OV5648MIPI_update_wb_register_from_otp(void)
{
	kal_uint16 temp, i, otp_index;
	struct OV5648MIPI_otp_struct current_otp;
	kal_uint32 R_gain, B_gain, G_gain, G_gain_R,G_gain_B;

	SENSORDB("OV5648MIPI_update_wb_register_from_otp\n");
	...
	if( 3 == i)
		{
		 	SENSORDB("[OV5648MIPI_update_wb_register_from_otp]no valid wb OTP data!\r\n");
			return 1;
		}
...
#if defined(OV5648MIPI_USE_LENC_OTP)    //copy form OV5650 LENC OTP
...
kal_uint16 OV5648MIPI_check_otp_lenc(kal_uint16 index)
{
   kal_uint16 temp,flag;
   kal_uint32 address;

   address = 0x20 + index*71;
   OV5648MIPI_write_cmos_sensor(0x3d00,address);
   
   flag = OV5648MIPI_read_cmos_sensor(0x3d04);
   flag = flag & 0xc0;

   OV5648MIPI_write_cmos_sensor(0x3d00,0);

   if(NULL == flag)
   	{
   		SENSORDB("[OV5648MIPI_check_otp_lenc]index[%x]read flag[%x][0]\n",index,flag);
   	    return 0;
   	}
   else if(0x40 == flag)
   	{
   		SENSORDB("[OV5648MIPI_check_otp_lenc]index[%x]read flag[%x][2]\n",index,flag);
   	    return 2;
   	}
   else
   	{
   		SENSORDB("[OV5648MIPI_check_otp_lenc]index[%x]read flag[%x][1]\n",index,flag);
		return 1;
   	}
}


kal_uint16 OV5648MIPI_read_otp_lenc(kal_uint16 index,struct OV5648MIPI_otp_struct *otp)
{
	...
	for(i = 0; i < 36; i++)
		{
			OV5648MIPI_write_cmos_sensor(0x3d00,address);
			otp->lenc_g[i] = OV5648MIPI_read_cmos_sensor(0x3d04);
			
			SENSORDB("[OV5648MIPI_read_otp_lenc]address[%x]otp->lenc_g[%d][%x]\n",address,i,otp->lenc_g[i]);
			address++;
		}
	//read lenc_b
	for(i = 0; i <8; i++)
		{
			OV5648MIPI_write_cmos_sensor(0x3d00,address);
			temp1 = OV5648MIPI_read_cmos_sensor(0x3d04);

			SENSORDB("[OV5648MIPI_read_otp_lenc]address[%x]temp1[%x]\n",address,temp1);

			address++;
			OV5648MIPI_write_cmos_sensor(0x3d00,address);
			temp2 = OV5648MIPI_read_cmos_sensor(0x3d04);

			SENSORDB("[OV5648MIPI_read_otp_lenc]address[%x]temp2[%x]\n",address,temp2);
...
//return 0
kal_uint16 OV5648MIPI_update_lenc(struct OV5648MIPI_otp_struct *otp)
{
	kal_uint16 i, temp;
	//lenc g
	for(i = 0; i < 36; i++)
		{
		 	OV5648MIPI_write_cmos_sensor(0x5800+i,otp->lenc_g[i]);
			
			SENSORDB("[OV5648MIPI_update_lenc]otp->lenc_g[%d][%x]\n",i,otp->lenc_g[i]);
		}
	//lenc b
	for(i = 0; i < 25; i++)
		{
			OV5648MIPI_write_cmos_sensor(0x5824+i,otp->lenc_b[i]);
			SENSORDB("[OV5648MIPI_update_lenc]otp->lenc_b[%d][%x]\n",i,otp->lenc_b[i]);
		}
	//lenc r
	for(i = 0; i < 25; i++)
		{
			OV5648MIPI_write_cmos_sensor(0x583d+i,otp->lenc_r[i]);
			SENSORDB("[OV5648MIPI_update_lenc]otp->lenc_r[%d][%x]\n",i,otp->lenc_r[i]);
		}
	return 0;
}
...
kal_uint16 OV5648MIPI_update_lenc_register_from_otp(void)
{
	kal_uint16 temp,i,otp_index;
    struct OV5648MIPI_otp_struct current_otp;
	...
	for(i = 0; i < 3; i++)
		{
			temp = OV5648MIPI_check_otp_lenc(i);
			if(2 == temp)
				{
					otp_index = i;
					break;
	...
	if(3 == i)
		{
		 	SENSORDB("[OV5648MIPI_update_lenc_register_from_otp]no valid wb OTP data!\r\n");
			return 1;
		}
	OV5648MIPI_read_otp_lenc(otp_index,&current_otp);

	OV5648MIPI_update_lenc(&current_otp);

	//at last should enable the shading enable register
	OV5648MIPI_read_cmos_sensor(0x5000);
	temp |= 0x80;
	OV5648MIPI_write_cmos_sensor(0x5000,temp);
...
UINT32 OV5648MIPIOpen(void)
{
    ...
    if (sensor_id != OV5648MIPI_SENSOR_ID || (mt_get_gpio_in(GPIO_MCAM_ID) != 1)){
...
static UINT32 OV5648GetSensorID(UINT32 *sensorID) 
{
    ...
    *sensorID=((OV5648MIPI_read_cmos_sensor(0x300A) << 8) | OV5648MIPI_read_cmos_sensor(0x300B));   
    ...
    if (*sensorID != OV5648MIPI_SENSOR_ID || (mt_get_gpio_in(GPIO_MCAM_ID) != 1)) {
