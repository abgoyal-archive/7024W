...
//add by rzwang for calibration in calling.
typedef enum
{
    CALIBRATION_POWER_ON,
    CALIBRATION_CALLING,
    CALIBRATION_OK,
    CALIBRATION_UNKNOW
}CALIBRATION_TAG;

CALIBRATION_TAG CALIBRATION_TAG_VAR = CALIBRATION_UNKNOW;
...
int cm36283_enable_ps(struct i2c_client *client, int enable)
{
    ...
    if(enable == 1){
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps enable_ps\n");
    #endif
        databuf[0]= CM36283_REG_PS_CONF3_MS;
        res = CM36283_i2c_master_operate(client, databuf, 0x201, I2C_FLAG_READ);
        
        if(res < 0){
            APS_ERR("cm36283_enable_ps:i2c_master_send function err\n");
            goto ENABLE_PS_EXIT_ERR;
        }
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps:CM36283_REG_PS_CONF3_MS value value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
    #endif
    ...
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps:CM36283_REG_PS_CANC value value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
    #endif
    ...
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps:CM36283_REG_PS_CONF1_2 value value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
    #endif
        //john0x03
        databuf[2] = 0x43;
        databuf[1] = 0xC2; 
        databuf[0]= CM36283_REG_PS_CONF1_2;
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps: CM36283_REG_PS_CONF1_2's databuf[1] = %x\n",databuf[1]);
    #endif 
    ...
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps: obj->ps_thd_val_low = %d, obj->ps_thd_val_high = %d\n",
                        atomic_read(&obj->ps_thd_val_low), atomic_read(&obj->ps_thd_val_high));
    #endif
    
        if(CALIBRATION_TAG_VAR == CALIBRATION_CALLING){
            u8 prox_data[PS_GET_NUM];
            int prox_sum = 0, prox_mean = 0, prox_max = 0,prox_min = 255;
        ...
        #ifdef CM36283_DEBUG
            APS_DBG("cm36283_enable_ps:CALIBRATION_TAG_VAR == CALIBRATION_CALLING!!!\n");
        #endif
            ...
            for(i = 0; i < PS_GET_NUM; i++){           
                if((err = cm36283_read_ps(client, &prox_data[i]))){
                    APS_ERR("cm36283_enable_ps: cm36283_read_ps---fail: %d\n", i);
                    return err;
                }

                if(0 == prox_data[i]){
                    invalid++;
					APS_DBG("cm36283_enable_ps:invalid = %d, i = %d\n", invalid, i);
                }
                else{    
                    ...
                    APS_DBG("cm36283_enable_ps: prox_data[%d] = %d\n", i ,prox_data[i]);
            ...
            if(PS_GET_NUM != invalid){
                prox_mean = prox_sum/(PS_GET_NUM-invalid); //get the ave value.
                prox_threshold_hi = prox_mean+10;
		   prox_threshold_lo = prox_mean+5;
            }
            
            if(/*(prox_mean < 15) || */(prox_min > 128) || (prox_max == 255) || (prox_threshold_hi >= 80 /*obj->hw->als_threshold_high == 80*/)) {  
                ...
                CALIBRATION_TAG_VAR = CALIBRATION_OK;
            }

		#ifdef CM36283_DEBUG
			APS_DBG("cm36283_enable_ps:CALIBRATION_CALLING:prox_min=%d,prox_max=%d,prox_sum=%d,invalid=%d,valid_number=%d\n",
					prox_min, prox_max, prox_sum, invalid, PS_GET_NUM-invalid);
		#endif
            APS_DBG("cm36283_enable_ps:CALIBRATION_CALLING:prox_mean = %d, obj->ps_thd_val_low = %d, obj->ps_thd_val_high = %d\n",
                     prox_mean, atomic_read(&obj->ps_thd_val_low), atomic_read(&obj->ps_thd_val_high));
            ...
            if(0 == obj->hw->polling_mode_ps){
                ...
                if(res <= 0){
                    APS_ERR("rzwang:cm36283_enable_ps:i2c_master_send function err\n");
                    goto ENABLE_PS_EXIT_ERR;
                }
                else{
                    APS_DBG("rzwang:cm36283_enable_ps:CM36283_REG_PS_THD write ok!!!\n");
                }   
            }
        }
    }
    ...
    #ifdef CM36283_DEBUG
        APS_DBG("cm36283_enable_ps: disable_ps\n");
    #endif
        databuf[0]= CM36283_REG_PS_CONF1_2;
        res = CM36283_i2c_master_operate(client, databuf, 0x201, I2C_FLAG_READ);
...
static int cm36283_get_ps_value(struct cm36283_priv *obj, u8 ps)
{
	...
	val = 0;
      ...
      APS_DBG("cm36283_get_ps_value:  ps_thd_val_high = %d  ps_thd_val_low = %d  ps = %d\n" ,
...
static ssize_t cm36283_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "(cm36283_obj->i2c_retry = %d  cm36283_obj->als_debounce = %d  cm36283_obj->ps_mask = %d\n  cm36283_obj->ps_thd_val = %d  cm36283_obj->ps_debounce = %d\n", 
...
static ssize_t cm36283_store_send(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	else if(2 != sscanf(buf, "addr = %d cmd = %d", &addr, &cmd))
	{
...
static ssize_t cm36283_store_recv(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	else if(1 != sscanf(buf, "addr = %d", &addr))
	{
		APS_ERR("invalid format: '%s'\n", buf);
		return 0;
	}
...
static ssize_t cm36283_show_status(struct device_driver *ddri, char *buf)
{
	...
	if(cm36283_obj->hw)
	{
		len += snprintf(buf+len, PAGE_SIZE-len, 
				"CUST: cm36283_obj->hw->i2c_num = %d,  (cm36283_obj->hw->power_id = %d cm36283_obj->hw->power_vol = %d)\n", 
				cm36283_obj->hw->i2c_num, 
				cm36283_obj->hw->power_id, 
				cm36283_obj->hw->power_vol);
	}
	else
	{
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "REGS: als_cmd_val= %d ps_cmd_val = %d ps_thd_val = %d ps_thd_val_high = %d ps_thd_val_low = %d enable = %d pending_intr = %d\n", 
				atomic_read(&cm36283_obj->als_cmd_val), 
				atomic_read(&cm36283_obj->ps_cmd_val), 
				atomic_read(&cm36283_obj->ps_thd_val),
				atomic_read(&cm36283_obj->ps_thd_val_high),
				atomic_read(&cm36283_obj->ps_thd_val_low),
				cm36283_obj->enable, 
				cm36283_obj->pending_intr);
...
int cm36283_write_ps_conf(struct i2c_client *client, u8 *data1, u8 *data2)
{
    int res=0;
    u8 databuf[3];

    APS_DBG("cm36283_write_ps_conf!\n");

    databuf[0] = CM36283_REG_PS_CONF1_2;
    databuf[1] = *data1;
    databuf[2] = *data2;

    APS_DBG("cm36283_write_ps_conf:data1 = %d  data2 = %d\n", *data1, *data2);
    APS_DBG("cm36283_write_ps_conf:databuf1[0] = %d  databuf2[1] = %d\n", databuf[1], databuf[2]);

    res = CM36283_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);

    APS_DBG("cm36283_write_ps_conf res = %d\n", res);

    if(res <= 0){
        APS_ERR("cm36283_write_ps_conf function err\n");
    }
}
//read interface
int cm36283_read_ps_conf(struct i2c_client *client, u8 *buf)
{
    int res=0;
    u8 databuf[2];
    ...
    if(res <= 0){
        APS_ERR("cm36283_read_ps_conf function err\n");
    }    

    snprintf(buf, PAGE_SIZE, "ps_conf1 = 0x%x, ps_conf2 = 0x%x\n", databuf[0],databuf[1]);

    return res;
}

static ssize_t cm36283_show_ps_conf_1_2(struct device_driver *ddri, const char *buf)
{
    int res=0;
    ...
    res=cm36283_read_ps_conf(cm36283_obj->client, buf);

    if(res <=0){
        APS_DBG("cm36283_read_ps_conf error!\n");
        return 0;
    }
...
static ssize_t cm36283_store_ps_conf_1_2(struct device_driver *ddri, const char *buf)
{
    int res = 0;
    u8 data[2];  

    APS_DBG("cm36283_store_ps_conf_1_2 is called!\n");
    
    if(2 == sscanf(buf, "%d  %d", &data[0], &data[1])){ 
        APS_DBG("cm36283_store_ps_conf_1_2:buf = %s\n", buf);
        APS_DBG("cm36283_store_ps_conf_1_2:data[0] = %x  data[1] = %x\n", data[0], &data[1]);
        res = cm36283_write_ps_conf(cm36283_obj->client, &data[0], &data[1]);

        APS_DBG("cm36283_store_ps_conf_1_2: res  = %d\n", res);
        ...
        APS_ERR("invalid content: input the number as  --- num1 num2 \n");
    }
...
static DRIVER_ATTR(ps_conf_1_2,     S_IWUSR | S_IRUGO, cm36283_show_ps_conf_1_2, cm36283_store_ps_conf_1_2);
/*----------------------------------------------------------------------------*/
...
static int cm36283_check_intr(struct i2c_client *client) 
{
    ...
    APS_LOG("cm36283_check_intr:CM36283_REG_PS_DATA value---value_low = %d, value_reserve = %d\n",databuf[0],databuf[1]);

#ifdef CM36283_DEBUG
    APS_DBG("cm36283_check_intr: ps = %d, obj->ps_thd_val_low = %d, obj->ps_thd_val_high = %d\n",
                    databuf[0], atomic_read(&obj->ps_thd_val_low), atomic_read(&obj->ps_thd_val_high));
#endif
    ...
    APS_LOG("cm36283_check_intr:CM36283_REG_INT_FLAG value---value_low = %d, value_high = %d\n",databuf[0],databuf[1]);

    if(databuf[1]&0x02)
    {
    intr_flag = 0;//for close
      APS_LOG("cm36283_check_intr: intr_flag = 0, for close!!!\n");       
    }else if(databuf[1]&0x01)
    {
    intr_flag = 1;//for away
      APS_LOG("cm36283_check_intr: intr_flag = 1, for away!!!\n");       
    }else{
    res = -1;
...
static void cm36283_eint_work(struct work_struct *work)
{
	...
	APS_DBG("cm36283_eint_work called!!!\n");
...
static void cm36283_prox_calibration(struct i2c_client *client)
{
    ...
    for(i = 0; i < PS_GET_NUM; i++)
    {
        if((err = cm36283_read_ps(client, &prox_data[i])))
        {
            APS_ERR("cm3623_prox_calibration:cm36283_read_ps fail: %d\n", i);
            return err;
        } 
        if(0 == prox_data[i]){
            invalid++;
			APS_DBG("cm36283_prox_calibration:invalid = %d, i = %d\n", invalid, i);
        }
        else{    
            if(prox_data[i] < prox_min)
            prox_min = prox_data[i]; //get the min value
            ...
            APS_DBG("cm36283_prox_calibration: prox_data[%d] = %d\n", i ,prox_data[i]);
            prox_sum += prox_data[i];
        }
        mdelay(10);
    }
    ...
    APS_DBG("cm36283_enable_ps:CALIBRATION_CALLING:prox_min=%d,prox_max=%d,prox_sum=%d,invalid=%d,valid_number=%d\n",                
              prox_min, prox_max, prox_sum, invalid, PS_GET_NUM-invalid);                                                              
#endif
    APS_DBG("cm36283_prox_calibration: prox_mean = %d, prox_threshold_lo = %d, prox_threshold_hi = %d\n", 
                    prox_mean, prox_threshold_lo, prox_threshold_hi);
...
static int cm36283_init_client(struct i2c_client *client)
{
	...
	int res = 0;
  #ifdef CM36283_DEBUG
      APS_DBG("cm36283_init_client is called!\n");
  #endif
	databuf[0] = CM36283_REG_ALS_CONF;
  ...
  #ifdef CM36283_DEBUG
	APS_DBG("cm36283_init_client:CM36283_REG_PS_CONF1_2 databuf[1] =%x\n");
  #endif
	//john0x03
	databuf[0] = CM36283_REG_PS_CONF1_2;
	databuf[1] = 0xC2;
	...
	if(res <= 0)
	{
		APS_ERR("cm36283_init_client:i2c_master_send function err\n");
		goto EXIT_ERR;
	}
...
#ifdef POWER_ON_CALI
       CALIBRATION_TAG_VAR = CALIBRATION_POWER_ON;
       cm36283_prox_calibration(client);
       CALIBRATION_TAG_VAR = CALIBRATION_CALLING;
#endif 

	if(0 == obj->hw->polling_mode_als){
              #ifdef CM36283_DEBUG
                   APS_DBG("cm36283_init_client:obj->hw->polling_mode_als is 0!\n");
              #endif
...
static int cm36283_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  #ifdef CM36283_DEBUG
	APS_DBG("cm36283_i2c_probe:cm36283_init_client() OK!\n");
  #endif
	if((err = misc_register(&cm36283_device)))
	{
		APS_ERR("cm36283_i2c_probe:cm36283_device register failed\n");
		goto exit_misc_device_register_failed;
	}
    
  #ifdef CM36283_DEBUG
	APS_DBG("cm36283_i2c_probe:cm36283_device misc_register OK!\n");
  #endif
