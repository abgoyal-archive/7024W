...
static unsigned int jrd_check_type_cnt = 0; //Record the accdet module interruption time/*add for pr469500*/
...
void accdet_eint_work_callback(struct work_struct *work)
{
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
        ...
        jrd_check_type_cnt = 0; /*add for pr469500*/
...
#define PIN_ADC_CHANNEL 5
    ...
    jrd_check_type_cnt++;/*add for pr 469500*/
	...
	case HOOK_SWITCH:
            ...
            else if(current_status == 1)
            {
                ...
                ACCDET_DEBUG("[Accdet] jrd jrd_check_type_cnt: %u", jrd_check_type_cnt);

                if(cable_type == HEADSET_NO_MIC && jrd_check_type_cnt < 4) { /*add for pr469500*/
