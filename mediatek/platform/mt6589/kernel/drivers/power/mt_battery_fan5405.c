...
//#define __TEST_BATTERY_ID__
#endif

/*bug529007,529010, ADC in range of (0, 1.5V), when registor 24k, VDD18, 
the battery id voltage is out of 1.5V, so compensate it. */
#define __BATT_ID_ADC_COMPENSATION__
...
#define CHG_MIN_BYD_BATT_ID  740
#define CHG_MAX_BYD_BATT_ID  920
#define CHG_MIN_LISHEN_BATT_ID  960
#define CHG_MAX_LISHEN_BATT_ID  1100
#define CHG_MIN_SCUD_BATT_ID  950	//1250, fix battery id to (950~1400), so actually remove check lishen battery
#define CHG_MAX_SCUD_BATT_ID  1400

#ifdef __TEST_BATTERY_ID__
//it is used to debug batt_id check when charge by software
#define DEFAULT_BATTER_ID 800
int g_battery_id = DEFAULT_BATTER_ID;
#endif

#ifdef __BATT_ID_ADC_COMPENSATION__
...
void check_battery_id(void)
{
	...
	printk("battery ADC compensation value is %d\n",(fg_current_temp*fg_r_value)/1000);
	if(!id_buffuer_first)
	{
	...
	if (DEFAULT_BATTER_ID != g_battery_id) {
		bat_id_vol = g_battery_id;
	}

	printk(KERN_INFO "BAT_ID_CHECK-------[ADB write battery id]g_battery_id:%d!\n", g_battery_id);
#endif
...
#if 0 //bug529007,529010, fix scud battery id, enlarge it, at the same remove lishen battery check.
...
#else        
		//Fix charging current to 500mA, yet?
//            fan5405_config_interface_liao(0x01,0xf8);
                fan5405_config_interface_liao(0x01,0x78);
#endif            
            if (Enable_BATDRV_LOG == 1) {
...
//bug529007,529010, check battery id when check battery status, if battery id is invalid, return PMU_STATUS_FAIL to deal it as battery failed.
#ifdef BATT_ID_CHECK_SUPPORT
    check_battery_id();
    if(0 == BMT_status.bat_id_valid) {
        return PMU_STATUS_FAIL;
    }
#endif
    ...
    {        
...
#endif            
        {
            ...
            // TODO: fake to  re-charger when battery volgate below RECHARGING_VOLTAGE
            pchr_turn_off_charging_fan5405();
            msleep(500);
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
		...
		pchr_turn_off_charging_fan5405();
		g_Charging_Over_Time = 1;   //zhaiqingbo add for charging fatctory PR512944 , 2013.8.22
		printk("turn_off_charging for factory\n");
		break;
         case TURN_ON_CHARGING_FOR_FACTORY:
		//Bug566029, in factory charging mode, if voltage is lower than threshshold, it should turn on to re-charge. 
		printk("pchr_turn_on_charging_fan5405 for factory\n");
		g_Charging_Over_Time = 0;
		pchr_turn_on_charging_fan5405();
		break;
        default:
...
//bug529007,529010, check battery id when probe, if battery id is invalid, it will lead to power off MS
#ifdef BATT_ID_CHECK_SUPPORT
    check_battery_id();
    if((0 == BMT_status.bat_id_valid)
