...
/*pcyang added for the capacity is different when reboot hanset--2013-09-27*/
...
//4. update DOD0 after booting Xs
	...
	if(abs(UbVbat-gFG_voltage)>100
		&& (UbVbat >= 3200) //bug546158, make sure UbVbat that from lk is valid
	)
        { 
		  printk("<0>~~Set hw_ocv(%d) to UbVbat(%d)\n",gFG_voltage,UbVbat);
		  gFG_voltage = UbVbat;
		  gFG_capacity_by_v = fgauge_read_capacity_by_v();
        }
	...
	gFG_voltageVBAT, gFG_voltage, gFG_capacity_by_v);
        ...
        {
            if( ( g_rtc_fg_soc != 0                     ) &&                    
                ( (g_rtc_fg_soc-gFG_capacity_by_v) < 10 ) &&
        ...
        {
            if( ( g_rtc_fg_soc != 0                     ) &&                    
                ( (gFG_capacity_by_v-g_rtc_fg_soc) < 10 ) &&
                (( gFG_capacity_by_v > 5 || upmu_is_chr_det() == KAL_TRUE ) ||
                (g_boot_reason == BR_WDT_BY_PASS_PWK)) 
                )
            {
