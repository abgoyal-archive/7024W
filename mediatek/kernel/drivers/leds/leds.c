...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
		...
		{
			printk("guoyanchun   MT65XX_LED_PMIC_BUTTON  6589");
			if (level) 
			{
				upmu_set_kpled_dim_duty(0x2);
				upmu_set_kpled_dim_div(2);
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	{	printk("guoyanchun   MT65XX_LED_PMIC_BUTTON  6577");
		if (level) 
		{
...
static void mt65xx_led_work(struct work_struct *work)
{
	...
	LEDS_DEBUG("[mt65xx_leds_probe] notify_led_type_enable:%d\n", notify_led_type_enable);	
