...
include device/jrdcom/rav4/jb_rav4.mk
...
.PHONY : new newall remake remakeall clean cleanall \
         ...
         update-api modem-info bindergen clean-modem perso check-sign copy-modem modem
...
check-dep: copy-makefile custgen
...
copy-makefile:
	@echo check-dep start...
	@echo mediatek/custom/common/modem/$(strip $(CUSTOM_MODEM))/modem.mak
	@echo MTK6589_modem/make/$(call Upper,$(strip $(CUSTOM_MODEM))).mak
	$(hide) cp MTK6589_modem/make/$(call Upper,$(strip $(CUSTOM_MODEM))).mak mediatek/custom/common/modem/$(strip $(CUSTOM_MODEM))/modem.mak
	$(hide) cat MTK6589_modem/make/custom_config/$(call Upper,$(strip $(CUSTOM_MODEM)))_EXT.mak >> mediatek/custom/common/modem/$(strip $(CUSTOM_MODEM))/modem.mak

copy-modem:
	@if [ -e $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM)) ]; then rm -R $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM)); fi
	@if [ -e $(MTK_ROOT_CUSTOM)/out/$(PROJECT)/modem ]; then rm -R $(MTK_ROOT_CUSTOM)/out/$(PROJECT)/modem; fi
	@cp -r $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG) $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM))
	@cp -r $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG) $(MTK_ROOT_CUSTOM)/out/$(PROJECT)/modem
...
update-modem: build-modem clean-modem custgen check-modem sign-modem
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
...
perso:  build-modem clean-modem custgen javaoptgen ptgen check-modem sign-modem
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) /usr/bin/perl mediatek/build/tools/mtkBegin.pl $(PROJECT)
...
#added by dfg for building modem. 2013.11.23
modem:
ifeq ($(BUILD_MODEM),yes)
	$(hide) echo $(SHOWTIME) $(SHOWBUILD)ing $@...
...
ifneq ($(ACTION), )
	$(hide) cd $(MODEM_WD) && \
	  (./make.sh $(call Upper,$(strip $(CUSTOM_MODEM))).mak clean $(DEAL_STDOUT) && \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION) || \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(MODULE_LOG) $(ACTION)) && cd $(MKTOPDIR)
else	
	$(hide) cd $(MODEM_WD) && \
	(./make.sh $(call Upper,$(strip $(CUSTOM_MODEM))).mak new $(DEAL_STDOUT) && \
	  cd .. && make -fmediatek/build/makemtk.mk copy-modem && \
...
#added by dfg for building modem. 2013.11.23
build-modem:
	$(hide)./makeMtk $(FULL_PROJECT) n modem
